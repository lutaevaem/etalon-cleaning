  ...
  return (
    <>
      <header className="site-header">
        ...
        <nav className={`main-nav ${menuOpen ? 'open' : ''}`}>{(content.nav || []).map((item) => <a href={item.href} key={item.href}>{item.label}</a>)}</nav>
      ...
      <main id="top">
        ...
        <section className="section quiz" id="quiz"><div className="quiz-card"><div className="quiz-copy"><p className="eyebrow">{content.quiz.eyebrow}</p><h2>{content.quiz.title}</h2><p>{content.quiz.text}</p></div><form className="quiz-form" onSubmit={handleSubmit}>
          ...
          <p className="form-note">Нажимая кнопку «{content.quiz.button}», вы соглашаетесь с <a href="/consent.html" target="_blank" rel="noreferrer">обработкой персональных данных</a> и принимаете <a href="/privacy.html" target="_blank" rel="noreferrer">Политику защиты и обработки персональных данных</a>.</p>
